...
const addCliente = async (req, res) => {
    ...
    try {
        ...
        db.query(checkQuery, [nombre, telefono], (error, results) => {
            ...
            db.query(insertQuery, [nombre, telefono], (insertError, result) => {
                if (insertError) {
                    console.error("Error al ingresar el cliente", insertError);
                    return res.status(500).json({ error: "Error al registrar el cliente" });
                }
        ...
        console.error("Error inesperado al ingresar el cliente", err);
        res.status(500).json({ error: "Error interno del servidor" });
    }
};*/
...
const addCliente = async (req, res) => {
    ...
    try{
        ...
        db.query(query, values, (error, result) => {
            if (error) {
                console.error("Error al ingresar un cliente", error);
                return res.status(500).json({ error: "Error al registrar el cliente" });
            }
            res.status(201).json({ message: "Cliente ingresado correctamente" });
        });
    }catch(err){
        console.error("Error al ingresar el cliente", err);
        res.status(500).json({ error: "Error interno del servidor" });
    }
};*/
